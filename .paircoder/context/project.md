...
- **Auth:** WebAuthn/Passkey (`@simplewebauthn/server` + `/browser`), JWT (7-day HS256, HttpOnly cookie)
- **Database:** Supabase (PostgreSQL + Supabase Storage)
- **Bank sync:** Teller.io (webhooks + polling)
- **AI:** Claude API (`claude-sonnet-4-6`) — transaction classification + vision schedule parsing
- **Hosting:** Vercel
